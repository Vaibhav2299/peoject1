...
public class forgotpassEmp extends JFrame implements ActionListener {
	...
	public static void main(String[] args) {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					forgotpassEmp frame = new forgotpassEmp();
		...
		btnGetPassword.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(e.getSource()==btnGetPassword)
				{
					...
					if(b)	
					{				
					String st="select password from employee where email=? and secques=? and secans=?";
					try
					{
			...
			boolean check(String user,String ques,String passwordField )
			{
				
				if((user).isEmpty() || ques.isEmpty() || passwordField.isEmpty() )
				{
					JOptionPane.showMessageDialog(null, "please enter all the fields", "project",0);
					return false;
				}
				String st="select * from employee where email=?;";
				try{
						...
						if(rs.next())
						{
							while(rs.next())
							{
								...
								if(!passwordField_1.equals(a) || !ques.equals(q))
								{
